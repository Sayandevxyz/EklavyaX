...
const ALL_COURSES_CATALOG = {
  ...
  social_science_lens: {
    id: "social_science_lens",
    category: "social_science",
    grade: "all",
    icon: "fa-globe-asia",
    title: "Social Science: Indian History & Physical Geography (EklavyaLens)",
    teacher: "Prof. Rajeshwar Sharma & EklavyaLens AI",
    rating: "4.9/5",
    totalLessons: 24,
    completedLessons: 14,
    progress: 60,
    lensUrl: "eklavyalens.html",
    description: "Explore Indian physical geography, river channels, Himalayan passes, Harappan civilization, and imperial battlefields through interactive maps.",
    currentLesson: {
      number: 15,
      title: "Indo-Gangetic Plains & Imperial Battlegrounds (Panipat & Magadha)",
      duration: "30 mins",
      summary: "Understand how fertile river basins shaped agricultural wealth and strategic warfare in Indian history.",
      keyPoints: [
        "Indo-Gangetic plain provided surplus food to sustain standing imperial armies.",
        "Panipat is the gateway plain between the Thar Desert and the Himalayas.",
        "Magadha's proximity to iron ore mines in Rajgir gave it military superiority."
      ]
    },
    chapters: [
      { name: "Unit 1: Northern Himalayan Frontiers & Mountain Passes (Khyber & Bolan)", lessons: 6, completed: true },
      { name: "Unit 2: Indus River Valley & Harappan Urban Planning", lessons: 6, completed: true },
      { name: "Unit 3: Indo-Gangetic Plains & Strategic Empires (Magadha, Panipat)", lessons: 6, completed: false, current: true },
      { name: "Unit 4: Peninsular Plateau, Western Ghats & Coastal Trade Routes", lessons: 6, completed: false }
    ]
  }
};
...
function filterByGrade(grade, btn) {
  ...
  document.querySelectorAll(".catalog-grade-btn").forEach(b => {
  ...
  if (btn) {
    ...
    btn.style.background = "var(--marigold)";
    btn.style.color = "#071510";
    btn.style.borderColor = "var(--marigold)";
  }
  filterCatalog();
}
...
function filterCatalog() {
  ...
  catalogGrid.innerHTML = filtered.map(c => {
    ...
    const badgeCategory = c.category === 'tech' ? 'badge-cs' : (c.category === 'social_science' ? 'badge-cs' : (c.category === 'humanities' ? 'badge-humanities' : `badge-${c.category}`));
      ...
      <div class="course-card">
        <div class="course-card-top">
          ...
          <div class="course-card-head">
            <span class="badge ${badgeCategory}">${c.category === 'social_science' ? 'Social Science' : c.category}</span>
        ...
        <div class="course-actions">
          ...
          `}
          ${c.lensUrl ? `
            <a href="${c.lensUrl}" class="btn-secondary" style="border-color: #38bdf8; color: #38bdf8; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; font-weight: 700; padding: 8px 12px; border-radius: 8px;">
              <i class="fas fa-globe-asia"></i> 3D Map
            </a>
          ` : ''}
